...
def get_args():
    ...
    parser = argparse.ArgumentParser(
            description="""---------------------------------------------------------------------------
    Filter_All_tsv - Filter all populations.haplotypes.tsv files to remove invariant, 'blank', and 
    ...
    cause singletons to be removed at this step.  This script is meant to run 
    on a directory (-i) containing the output of the Run_Stacks.py script, in which separate subdirectories
    were created for each population analysis (one for each -r or -R value). It will perform all filtering actions 
    on each of the haplotypes.tsv contained in each of the populations subdirectories. In each populations subdirectory, 
    the output files include a filtered tsv file ("populations_XX.haplotypes.filtered_#.tsv"), 
    a general log file ("Filter_All_tsv.#.log"), a snp site distribution file ("SNP_distributions.#.log"), 
    and two missing data files (Initial_Missing_Data_Per_Sample.#.log, Final_Missing_Data_Per_Sample.#.log). 
    In the input directory (-i), a main summary file ("Filter_All_tsv.summary.#.log") will be written. Portions 
    of the output file names (#) are filled by the argument values. This means different values of per sample 
    missing data (-m) can be run on the same input directory. Each -m value will produce unique output files 
    (rather than causing overwriting), which allows for easy comparison of the filtering effects. 
    ...
    parser.add_argument("-i", "--indir",
                            ...
                            "all of the populations output directories produced by Run_Stacks.py.")
...
def find_dirs(indir):
    ...
    Arguments:
    indir - Full path to a directory that contains the
...
def main():
    ...
    if os.path.isfile(SUMLOG):
        ...
        with open(SUMLOG, 'a') as fh:
            fh.write("Filter_All_tsv settings:\n\n-i (indir): {0}\n-m (missingdata): {1}\n-s "
                         "(snpselection): {2}\n\n{3}\n\n".format(argd["indir"], argd["missingdata"], argd["snpselection"], "="*80))
    ...
    for p in paths:
        ...
        tf2 = datetime.now()
        print_log(LOG, ("\n\n{}".format("="*80)))
        print_log(LOG, ("\nTotal elapsed time for {}: {} (H:M:S)\n".format(hapfile, tf2 - tb2)))
        print_log(LOG, ("{}\n\n".format("="*80)))
